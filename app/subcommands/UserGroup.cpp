...
CLI::App* setupShow(CLI::App* app, teams::GraphServiceClient& client) {
    ...
    auto* sub = app->add_subcommand("show", "Show group")
                    ->alias("sh")
...
CLI::App* setupDelete(CLI::App* app, teams::GraphServiceClient& client) {
    ...
    auto* sub = app->add_subcommand("delete", "Delete group")
                    ->alias("d")
                    ->callback([opt, &client]() { caUG::del(*opt, client); });
    sub->add_option("-n, --name", opt->name, "Name of the group")->required();
    return sub;
}
...
CLI::App* setupSend(CLI::App* app, teams::GraphServiceClient& client) {
    ...
    auto* sub = app->add_subcommand("send", "Send a message to a group")
                    ->alias("s")
